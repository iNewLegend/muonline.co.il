...
void CWinSocket::Startup(int aSocketType, int aProtoType) 
{
	if(WSAStartup(2, &m_WSAData) == 0)
	{
		if(aSocketType == SOCK_TCP && aProtoType == PROTO_TCP || aSocketType == SOCK_UDP && aProtoType == PROTO_UDP)
		{
			...
			else
			{
				...
				g_Setup.MsgBox("Can't create socket. Calling socket() failed -> %d", WSAGetLastError());
		...
		else
		{
				...
				g_Setup.MsgBox("Incompatible socket type with protocol type -> %d", WSAGetLastError());
	...
	else
	{
		...
		g_Setup.MsgBox(" Can't startup WSA library -> %d", WSAGetLastError());
...
void CWinSocket::Bind(USHORT aPort) 
{
	...
	else
	{
		...
		g_Setup.MsgBox("Can't bind socket. Calling bind() failed -> %d", WSAGetLastError());
...
void CWinSocket::Listen() {
	if(m_aSocketType == SOCK_TCP)
	{
		...
		else
		{
			...
			g_Setup.MsgBox("[PlayerSocket][Boost] :: Can't listen on socket. Calling listen() failed -> %d", WSAGetLastError());
	...
	else
	{
		...
		g_Setup.MsgBox("[Boost] :: Can't listen on UDP socket -> %d", WSAGetLastError());
...
bool CWinSocket::Connect(const char *sIpAddr, int aPort) 
{
	...
	else
	{
		...
		else
		{
			...
			g_Setup.MsgBox("Error on calling connect() -> %d", aWSAError);
...
void CWinSocket::SetBlocking(unsigned long uStatus) 
{
	if(ioctlsocket(m_uSocket, FIONBIO, &uStatus) == SOCKET_ERROR)
	{
		...
		g_Setup.MsgBox("Can't change socket blocking mode. Error on calling ioctlsocket() -> %d", WSAGetLastError());
...
void CWinSocket::AcceptMonitor() 
{
	...
	else
	{
		...
		g_Setup.MsgBox("Error on accepting new socket -> %d", WSAGetLastError());
