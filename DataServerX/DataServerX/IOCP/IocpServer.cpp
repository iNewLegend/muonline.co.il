...
public:
		...
		uint32_t numThread)
	{
		...
		catch (...)
		{
			...
			MessageBoxA(0, "Unable to initaizlie the icop\rmay happend bcoz of alrady used port", "error", MB_OK);
			// ----
			Sleep(5000);
			// ----
			ExitProcess(0);
...
//			throw CIocpException(tstring(_T("Connection does not exist")));
	...
	void Send(uint64_t cid, LPBYTE data, int size)
	{
		//std::vector<char> v(data, data + size);
		
		Send(cid, std::vector<uint8_t>(data, data + size));
	}
...
void CIocpServer::Send(uint64_t cid, LPBYTE data, int size)
{
	return m_impl->Send(cid, data, size);
}
